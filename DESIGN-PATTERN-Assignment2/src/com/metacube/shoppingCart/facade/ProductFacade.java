...
public class ProductFacade {
    ...
    private ProductFacade() throws IOException{
        ...
        productDao = (BaseDao<Product>) DaoFactory.getDaoForEntity(EntityName.PRODUCT, DBType.IN_MEMORY);    
    }
    ...
    public Map<String, Product> getAllProducts() throws IOException{
    	return (Map<String, Product>) productDao.getAll();
    }

	public double getProductPriceByProductId(String productId) {
		return ((InMemoryProductDao) productDao).getProductById(productId).getPrice();
	}

	public Product getProductById(String key) {
		return ((InMemoryProductDao) productDao).getProductById(key);
	}
}
